...
class Client extends EventEmitter {
  ...
  _performDefaultSegmentHandling(
    sender,
    adr,
    type,
    service,
  ...
  _processConfirmedServiceRequest(
    address,
    type,
    service,
    maxSegments,
    ...
    if (service === baEnum.ConfirmedServiceChoice.READ_PROPERTY) {
      ...
      service === baEnum.ConfirmedServiceChoice.DEVICE_COMMUNICATION_CONTROL
    ) {
      result = baServices.deviceCommunicationControl.decode(
        buffer,
        offset,
        length
      );
      ...
      service === baEnum.ConfirmedServiceChoice.CONFIRMED_EVENT_NOTIFICATION
    ) {
  ...
  _processUnconfirmedServiceRequest(
    address,
    type,
    service,
    buffer,
    ...
    if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
      ...
      service === baEnum.UnconfirmedServiceChoice.UNCONFIRMED_COV_NOTIFICATION
    ) {
      result = baServices.covNotify.decode(buffer, offset, length);
      if (!result)
      ...
      service === baEnum.UnconfirmedServiceChoice.UNCONFIRMED_EVENT_NOTIFICATION
    ) {
      result = baServices.eventNotifyData.decode(buffer, offset, length);
      ...
      service === baEnum.UnconfirmedServiceChoice.UNCONFIRMED_PRIVATE_TRANSFER
    ) {
      result = baServices.privateTransfer.decode(buffer, offset, length);
      if (!result) return debug("Received invalid privateTransfer message");
      this.emit("privateTransfer", {
  ...
  whoIs(options) {
    ...
    const npduType =
      this._settings.address !== this._transport.getBroadcastAddress()
        ? baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU
